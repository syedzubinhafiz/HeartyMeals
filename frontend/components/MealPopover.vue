<template>
  ...
  <div v-if="visible" class="meal-popover-overlay fixed inset-0 z-50 flex items-center justify-center p-4" @click="handleOverlayClick">
    <div class="meal-popover bg-[#E5D7BE] rounded-2xl shadow-2xl border border-[#d5c7ae] max-w-sm w-full mx-4 transform transition-all duration-200 scale-100" @click.stop>
      <!-- Header -->
      ...
      <div v-if="isToday && !mealInfo.is_consumed" class="p-6 pt-0">
        <div class="space-y-3">
          <button @click="editMeal" class="w-full flex items-center justify-center px-4 py-3 bg-teal-100 hover:bg-teal-200 text-teal-700 rounded-lg transition-colors font-medium shadow-md">
            <i class="fas fa-edit mr-2"></i>
            Edit Meal
          </button>
          <div class="flex space-x-3">
            <button @click="removeMeal" class="flex-1 flex items-center justify-center px-4 py-3 bg-red-100 hover:bg-red-200 text-red-700 rounded-lg transition-colors font-medium shadow-md">
              <i class="fas fa-trash text-red-700 mr-2"></i>
              Remove
            </button>
            <button v-if="!mealInfo.is_consumed" @click="consumeMeal" class="flex-1 flex items-center justify-center px-4 py-3 bg-green-100 hover:bg-green-200 text-green-700 rounded-lg transition-colors font-medium shadow-md">
      ...
      <div v-if="mealInfo.is_consumed" class="px-6 pb-6">
        <div class="bg-white text-green-700 text-center py-3 rounded-lg font-medium shadow-md">
          <i class="fas fa-check-circle mr-2"></i>Consumed
        </div>
      </div>
    </div>
  </div>
...
.meal-popover-overlay {
  background-color: rgba(188, 170, 152, 0.5);
